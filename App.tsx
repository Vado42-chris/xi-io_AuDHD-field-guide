...
const FormattedText: React.FC<{ text: string }> = ({ text }) => {
  ...
  return (
    <div className="space-y-4">
      {segments.map((line, i) => {
        if (line.startsWith('### ')) return <h3 key={i} className="text-[10px] font-black uppercase text-[#4DB6AC] mt-6">{line.replace('### ', '')}</h3>;
        if (line.startsWith('## ')) return <h2 key={i} className="text-xl font-thin text-white mt-8">{line.replace('## ', '')}</h2>;
        return <p key={i} className="text-sm font-light leading-relaxed opacity-80">{line}</p>;
      })}
    </div>
  );
};
